...
TASKS = {
    ...
    # Added 2026-09-19 as a SECOND held-out task. `lift_peg_upright`, the tree's other
    # held-out task, cannot be expressed by a Cartesian-pose harness at all (its
    # motion-planning solution stands the peg up by pivoting it against the table while the
    # gripper itself rotates only 35 deg, so GT replay scores 0/20 -- see
    # eval/rollout_maniskill.py), which would have left the held-out-TASK block resting on
    # `pull_cube` alone. PlaceSphere is a pick-and-place, the class the harness handles
    # (pick_cube GT-replays 20/20), and it has an upstream motion-planning solution, so its
    # demonstrations are generated by the same solver as every other tree here.
    "PlaceSphere-v1": {
        "task_dir": "place_sphere",
        "center": [-0.03, 0.0, 0.03],
        "desc": [
            "put the sphere into the bin",
            "pick up the ball and place it in the bin",
            "place the sphere inside the container",
        ],
        "actor_roles": {
            "sphere": "distractor",     # promoted to target via seg_targets at load time
            "bin": "goal",
            "table-workspace": "background",
            "ground": "background",
        },
        "referring": {"sphere": ["sphere"], "bin": ["bin"]},
    },
    "PullCube-v1": {
        "task_dir": "pull_cube",
