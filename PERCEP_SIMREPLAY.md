...
如果代码有问题或者不完整，你就debug 然后写完整以后继续执行。
...
| 模态 | 专家 | 获取方式 | 权限 | 备注 |
|---|---|---|---|---|
| seg | **SAM 3** | HF `facebook/sam3` | **gated，已经通过access 申请** | Vision Banana 的主分割对照；原生支持文本提示，最该加 |
...
SAM 3 这个模型的access 已经被granted 了，，除此之外，你先跑ungate 的模型，gated 的模型可以放到最后跑，如果未来遇到 gated 的模型（目前都有access 应该），停下来告诉用户去 HF 页面申请权限.

不要换一个近似的模型顶替。拿到权限后用 `hf auth login` 登录。
